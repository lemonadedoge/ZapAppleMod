...
public class BlockZapAppleFlowers extends Block
  implements IShearable
{
  ...
  public BlockZapAppleFlowers(int id)
  {
  ...
  private boolean canFlowerStay(World world, int x, int y, int z)
  {
    ...
    if (meta > 0)
    {
      for (int i = 0; i <= 3; i++)
      {
        ...
        if (((meta & var8) != 0) && (!canBePlacedOn(world.getBlockId(x + net.minecraft.src.Direction.offsetX[i], y, z + net.minecraft.src.Direction.offsetZ[i]))) && ((world.getBlockId(x, y + 1, z) != blockID) || ((world.getBlockMetadata(x, y + 1, z) & var8) == 0)))
        {
    ...
    if (var6 != meta)
    {
      world.setBlock(x, y, z, blockID, var6, 1);
    }
